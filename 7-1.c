...
long calcsize(const struct entry *en)
{
    if(en->children != NULL)
    {
        long size = 0;
...
long atleast(const struct entry *en, long n)
{
    if(en->children != NULL)
    {
        long size = 0, minsize = 99999999, dirsize;
        for(struct entry **it = en->children; it != en->chldlen + en->children; ++it)
        {
            dirsize = atleast(*it, n);
            if((*it)->children != NULL && dirsize >= n)
                minsize = minsize > dirsize ? dirsize : minsize;
            size += dirsize;
        }
        return minsize == 99999999 ? size : minsize;
    }
    return en->size;
}
int main(int argl, char *argv[])
{
    ...
    size = atleast(rootdir, calcsize(rootdir) - 40000000);
    freeen(rootdir);
    printf("%li\n", size);
    return 0;
}

...
unsigned check(const struct list *restrict l, const struct list *restrict r)
{
    return lscmp(l, r) < 0;
}
...
int cmphelp(const void *x, const void *y)
{
    const struct list *const *llsp = x, *const *rlsp = y;
    return lscmp(*llsp, *rlsp);
}
int main(int argl, char *argv[])
{
    char l[1000], r[1000];
    struct list *lsls[1000];
    unsigned ind = 0;
    unsigned t, s;
    struct list *tls = parse("[[2]]");
    struct list *sls = parse("[[6]]");
    ...
    lsls[ind++] = tls;
    lsls[ind++] = sls;
    qsort(lsls, ind, sizeof(struct list*), cmphelp);
